...
- **CI**: `verify`, `audit`, `actionlint`, `codeql`, `e2e` (Playwright + axe in both themes on desktop Chrome, and on iPhone WebKit; byte budgets; ADR-0011), `lighthouse` (ADR-0012), `links` (internal, offline; ADR-0013), and `pr-title` (commitlint on the PR title, which becomes the squash commit) on every PR. External links weekly (`links.yml`).
