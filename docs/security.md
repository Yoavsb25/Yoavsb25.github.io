...
- Fenced code blocks would break the policy: Astro's default Shiki highlighter writes inline `style` attributes, which the CSP blocks (the build warns about this). No case study has one yet; before adding one, switch to class-based highlighting (Prism or Shiki CSS-variable themes). The e2e CSP check fails on the first such page.
