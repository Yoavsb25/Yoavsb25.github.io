...
- Rejected: `npx -y @playwright/mcp@x.y.z` (unverified download on each cold cache); relying on agent prompts alone; hooks in agent frontmatter (did not fire for subagent tool calls in testing); prefix-regex command allowlists (bypassed by newlines, `&`, process substitution, and tool flags such as `rg --pre`).
