...
The user or main agent runs `npm run dev`. The site lives under its base path (`site.base`, currently `/`): navigate to http://localhost:4321/ (routes are `/<route>`); if it is not reachable, stop and say so. You have no shell access. Save screenshots under `.playwright-mcp/`.
