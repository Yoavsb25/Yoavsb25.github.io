...
.claude/hooks/       hook scripts; logic in lib/rules.mjs, tested in tests/unit/hooks.test.ts
...
| Before Edit/Write | `guard-protected.mjs` | Asks before editing workflows, `.claude/settings.json`, hooks, lockfile, `lefthook.yml`, `CNAME`; denies `.env*` |
| Before Edit/Write | `scan-secrets.mjs`    | Denies content that looks like an API key, token, or private key                                                 |
| Before Bash       | `guard-bash.mjs`      | Denies push, `--no-verify`, hard reset, recursive force delete, global installs, piping downloads to a shell     |
...
`guard-bash` matches the whole command text, so a command that merely mentions a blocked pattern (e.g. in a heredoc) is also denied — write the content with the Write tool instead.
Personal overrides go in `.claude/settings.local.json` (gitignored). Test fake secrets must be built at runtime (see `tests/unit/hooks.test.ts`).
